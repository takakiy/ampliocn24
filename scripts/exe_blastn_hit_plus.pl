...
if ($ARGV[0] =~ /-h/){
   print "EXEC OPTION;
            ...
            exec make_hitlist_folder; -rd blastout_file_path -sel txt
             \(exp. ~/folder/.*out\)\n";
   print "FILTER OPTION;           
...
use File::Basename;
	...
	$blast_home="$ENV{HOME}/biotools/local/homology/blast";
...
#================================================================================ 
sub main_control{
  my ($blastmode,$blastout,$lnum,$filters,$llen)=@_;
...
sub make_table_blastn { 
  ...
  my ($res);
   ...
   @sel_hitsbj_sort=sort {
 #          $a->[0]->[14] cmp $b->[0]->[14] or
           $b->[0]->[3] <=> $a->[0]->[3] or
