...
##   $blastout= <IN>;
   ...
   $error_que= &main_control($blastmode,$infile_path,$lnum,$filters,$lfram);
...
#####  $result_splits= &ToEach_blastout($blastout);
    ...
    while(<RE>) {
        ...
        if (/^Query\=/../^Effective search space used\:/) {
            ...
            if (/^Effective search space used\:/) {
                $one_blastout= join "\n",@resone;
                @resone= ();
