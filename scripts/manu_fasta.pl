...
$setmode= 0;
 ...
 if ((@ARGV == 0) || ($ARGV[0] =~ /\-h/)) {
  ...
  print "
	...
	## EXTRACT SPECIFIC SUBSEQ x
		ex.  manu_fasta.pl -e x -d dbseq -a 1 200 (-b 20 15 both term) -outform (fasta)/table)
            ---> rev_infile.fas rev_infile.txt
            -a  start end
            -n length of cuttin sequences

	## RENAME SEQ
		ex.  manu_fasta.pl -e v -d dbseq -l listfile[ORINAME(ID) NEW_NAME] ---> rev_infile.fas

	## SLICE a number of seq　(basepair)
    ...
    ## TO FASTQ
        ex.  manu_fasta.pl -e fastq -d dbseq ---> out_rev.fq
            
    ## TRIM
            ex.  manu_fasta.pl -e trim -d dbseq -trim 0 30 ---> out_trimm.fq
...
#  CONTROL EXEC MODE
#==========================================================================
$EM_MODE = 1 if ($exemode=~ /^([sdcrxnovma]|len|fastq|complement|trim)/);
...
### CUTTING
	$infile= $option->{-d}->[0];
	$inname= (split/\//,$infile)[-1];
    $outform= exists $option->{-outform} ? $option->{-outform}->[0] : 'fasta';
	
	($cinfo)=&cutting_sequence($seqinfo,$inname,$option,$outform);
	print "CUT_READ: $cinfo->{'y'} \/ $cinfo->{'c'}\n";

	print "OUTPUT: $cinfo->{out}\n";
...
#		$subseq= &limit_seqline($subseq);
	...
	($rcount)= &rename_sequence($seqinfo,$option->{-l}->[0]);

    print "RENAME: $rcount\n";
    print "OUT: rename_seq.fas\n";
    
} elsif ($exemode eq "m") {
	open (OUT,">out_subseq.fas");
    
    $getnum= $option->{-u}->[0];
    $num= 1;
    foreach $tax (@{$seqinfo->{tax}}) {
        $seq= $seqinfo->{seq}->{$tax};
        $subline= &limit_seqline($seq);
        print OUT ">$tax\n$subline\n";
        $num++;
	...
	print "SPLIT NUM ==> $getnum\n";
    print "OUTPUT: out_subseq.fas\n";
...
} elsif ($exemode eq "fastq") {
    open (OUT,">out_xxx.fq");
    foreach $taxone (keys %{$seqinfo->{seq}}) {
        $seq= $seqinfo->{seq}->{$taxone};
        $qual= "G" x length($seq);
 #       print "$taxone $seq $qual\n";
        print OUT "\@${taxone}\n$seq\n+\n$qual\n";
    }
    
    print "OUTPUT: out_xxx.fq\n";
    
} elsif ($exemode eq "complement") {
    open (OUT,">out_rev.fa");
    foreach $taxone (@{$seqinfo->{tax}}) {
        $seq= &complement_seq($seqinfo->{seq}->{$taxone});
        print OUT ">$taxone\n$seq\n";
    }
    
    print "OUTPUT: out_rev.fa\n";
    
} elsif ($exemode eq "trim") {
    open (OUT,">out_trimm.fa");
    ($trim_left,$trim_right)= @{$option->{-trim}};
    ...
    foreach $taxone (@{$seqinfo->{tax}}) {
        print "$taxone\n";
        $seq= $seqinfo->{seq}->{$taxone};
        $sublen= length($seq)-$trim_left-$trim_right;
        $subseq= substr($seq,$trim_left,$sublen);
        $subseq_ln= &multiline_seq($subseq);
        
        print OUT ">$taxone\n$subseq_ln\n";
    }
    
    print "OUTPUT: out_trimm.fa\n";
...
sub cutting_sequence {
	my($seqinfo,$inname,$option,$outform)= @_;
### CUTTING

    $cinfo= {};
    if ($outform eq "fasta") {
        $cinfo->{out}= "rev_cut_seq.fas";
        open (OUT,">rev_cut_seq.fas");
    } elsif ($outform eq "table") {
        $cinfo->{out}= "rev_cut_seq.txt";
        open (OUT,">rev_cut_seq.txt");
    }
	...
	foreach $tax (@{$seqinfo->{tax}}) {
		...
		if (exists $option->{-a}) {
			...
			if ($len > $p5cut + $p3cut) {
				$subseq_1= substr($seq,0,$p5cut);
				$subseq_2= substr($seq,$len-$p3cut,$p3cut);
			#	$sublen= length $subseq_2;
                 ...
                 if ($outform eq "fasta") {
                     ...
                     print OUT "$tax\t$subseq_1\t$subseq_2\n";
                }
				$cinfo->{y}++;
			}
		}
