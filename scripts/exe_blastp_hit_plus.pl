...
open (LOG,">out_exe_blastp_hit.logs");
...
@filepaths=();
	...
	$blast_home_plus="$ENV{HOME}/biotools/local/homology/ncbi-blast";
	$show_gis= "";
	$selkeywd= exists $option->{-sel} ? $option->{-sel}->[0] : '^\w+';

if (exists $option->{-e}){
     $infile= $option->{-e}->[0];
    if (exists $option->{-d}){
        $database=$option->{-d}->[0];
			$database= "${blast_home_plus}/db/$database" if ($database =~ /^nr|kegg|uniprot|uniprot_sprot$/);
    ...
    $blastout_file="temp_blastpout".$head;
    # NORMAL

		$cmd= "${blast_home_plus}/bin/blastp -query $infile -db $database -out $blastout_file";
		$cmd= $cmd." -num_descriptions 200 -num_alignments 200 -seg no -num_threads 6 -evalue 1e-5 $show_gis";
...
$resultout= exists $option->{-o} ? $option->{-o}->[0] : "hitlist_blastp".$head;
open (OUT, ">$resultout");
@title= ('#Query');
...
foreach $infile_path (@filepaths){
	$blastout_counter++;
	print "##FILE COUNTER: $blastout_counter\n";
...
#    $subfasta= &ToEach_blastout_new($blastout);
...
#		print "No: $counter\n";
     ...
     ($query_info,$hit,$hitresult)= &collect_hit_blastp([@$refhit]);
     ...
     $lnum= &make_table_blastp($query_info,$hit,$sel_hitsbj,$lnum);
...
sub make_table_blastp { 
  ...
  my ($res);
   ...
   ($qgene,$qlen,$qdes)= @$query_info; 
    $grpcount=0;
    if ($hit eq "HIT"){
       ($sbj_subsort)= &sort_qarea_blastp($sel_hitsbj,$qgene);
...
#	my (@outputs);
   ...
   open (CLOG,">out_check.logs");
	print CLOG "$outputs[0]\n";
...
#    my (@outputs);
    ...
    while(<IN>) {
        chomp;
        if (/^Query\= /../Effective search space used\:/) {
            if (/^Query\= /) {
                @resone= ();
                push @resone,$_;
            } elsif (/Effective search space used\:/) {
                push @resone,$_;
                $restxt= join "\n",@resone;
                @resone=();
                push @outputs,$restxt;
